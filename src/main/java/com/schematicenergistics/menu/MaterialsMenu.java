...
public class MaterialsMenu extends AEBaseMenu {
    ...
    private void sendMaterials(ServerPlayer player) {
        ...
        PacketHandler.sendToClient(new MaterialListClientPacket(0, 1, all), player);
    }

    private List<MaterialListEntry> buildMaterialsList() {
        var logic = getLogic();
        if (logic == null) return List.of();

        var gridNode = logic.getGridNode();
        if (gridNode == null || gridNode.getGrid() == null) return List.of();

        var grid = gridNode.getGrid();
        var storage = grid.getStorageService().getInventory();
        var craftingService = grid.getCraftingService();
        if (storage == null || craftingService == null) return List.of();

        var cannon = logic.getLinkedCannon();
        if (!(cannon instanceof ISchematicAccessor accessor)) return List.of();
        ...
        entries.sort(Comparator
                .comparingInt((MaterialListEntry e) -> {
                    if (e.available() >= e.required()) return 2;
